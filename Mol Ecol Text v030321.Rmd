...
The first day of class we will prep our computers - I'm sorry, ChromeOS I do not know how to install R onto - for using R/RStudio for a major resource in this class. If at all possible, before the class begins you should install R:
...
Please note the risk in all of this is that *packages* and *versions* of software are constantly changing, and sometimes code that has been working will stop (and vice-versa) because of these changes.
...
What does that mean? Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>. Adding the Shiny apps means that the document is *interactive*. The only downside is that it means that users must have **R** and **RStudio** installed, plus a few R packages, on their computer. The upside is that it is more of a living document. It means that as data change, the output of analysis can change. It also means that R code can be built directly into the document so that you can see how some figures or modules are generated, and you can build on this knowledge. You can embed an R code chunk like this:
...
For other users of this document, please note: I use a lot of examples I am familiar with, meaning they are often projects I'm an author on, or was on that person's committee, or whatever. There is so much other *fantastic* science out there, this isn't about ego though: it's just my ability to immediately dig deeper with those as examples not just of how the science *can* be done, but also about when it *could have been done better*. Also, I'm a marine ecologist; when I talk about plants, it is basically because of great colleagues who have entranced me with their weird terrestrial photosynthetic life, and mammals and fish similarly: I credit cool colleagues who have brought me into the fold. If you end up using this as an instructor, I encourage you to think about including your own, even better, examples.
...
*N.B.* I am aiming this at upper-level undergrads in ecology or evolutionary biology who may have had some introductory genetics or evolution; but, I am going to do my best to not assume you remember everything from those classes. Beginning grad students may also benefit, but should be encouraged to lead the class in paper discussions or experiential workshops to help build their depth of understanding. Also, with this being written in the work-at-home era of **COVID**, some references will be scant pointers to the actual resource and I hope you will forgive me when I know who I'm pulling from but can't find it right away.
...
![**Fig.1.2 - A pleco caught in the Chacamax River in Chiapas, Mexico - photo credit Krista Capps.**](/Users/johnwares/Dropbox/MolEcolText/MEImages/Pleco8-21.jpg)
...
![**Fig.2.1 - 9 gorgeous orange quadrats, photo credit the author but quadrats and note thanks to Dr. Marjorie Wonham, I believe. **](/Users/johnwares/Dropbox/MolEcolText/MEImages/marsquadrats.jpg)

## 2.1. Our sampling effort

If you aren't familiar with a *quadrat*, the photo above (Figure 2.1) includes "9 gorgeous orange quadrats" and you may quickly deduce that these are nothing more than PVC squares with grids installed using heavy fishing line, and the whole thing spray painted orange so they can be easily found in an intertidal marine habitat. A quadrat is a means to sample spatial diversity that is commonly used by ecologists to estimate the diversity of a larger spatial domain, with the scale varying depending on the system one is looking at, the evenness in that system, and the type of diversity to be counted (and thus extrapolated to say something about the whole ecosystem). 
...
![**Fig.2.3. DNA sequence alignment, with amino acids shown for protein sequence. The colored positions indicate mutational diversity where the less frequent variant is highlighted.**](/Users/johnwares/Dropbox/MolEcolText/MEImages/DNAalign.jpg) 
...
![**Fig.2.4. Illustration of the idealized barcoding gap (top panel) for contrasting diversity within and between species; the bottom panel shows that sometimes it gets more complicated. Just ask folks who study corals...e.g. Tonya Shearer's work.**](/Users/johnwares/Dropbox/MolEcolText/MEImages/MeyerPaulaygap.jpg) 
...
![**Fig.2.5a. A gene tree representation of the sequence similarity among mitochondrial sequences sampled from the barnacle Chthamalus fragilis on the east coast of North America.**](/Users/johnwares/Dropbox/MolEcolText/MEImages/fig-1-1x-2.jpg) 

![**Fig.2.5b. Spatial distribution of distinct phylogenetic clades shown in Fig.2.5a.**](/Users/johnwares/Dropbox/MolEcolText/MEImages/fig-3-1x-3.jpg)  
...
![**Fig.2.6 - A row of *C. fragilis* settled on a stem of the cordgrass *Spartina alterniflora*. Photo by Y. Zhang, GCE-LTER.**](/Users/johnwares/Dropbox/MolEcolText/MEImages/Cfrag.jpg)
...
# Box 1. An aside to explain these data better and how we obtain them. {#Box1}
...
![**Fig.B1 - From https://www.illumina.com/science/technology/next-generation-sequencing/ngs-vs-sanger-sequencing.html, a rough guideline to the benefit of massively parallel "non-Sanger" seqeuncing as the number of focal loci increases.**](/Users/johnwares/Dropbox/MolEcolText/MEImages/cost-effectiveness-of-targeted-ngs.jpg)
...
# 3. A bit more biological reality, and how diversity is generated
...
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/eucalyptus.jpg')
```
...
```{r driftsim2}
#library(learnPopGen)
#driftplot<-drift.selection(p0=0.2,Ne=500,w=c(1,1,1),ngen=2000,nrep=15)
#this is terrible slow code
library(ggplot2)

#shinyAppFile("/Users/johnwares/Dropbox/MolEcolText/shiny_popgen-master/Drift/drift_app.R",options=list(width="100%",height=600))
...
Your task with this simulator is to learn about a key understanding about random genetic drift. Remember, at this point in time all diversity has no effect on **fitness**. You can see from the app above that you can change population size (*N*), the initial allele frequency, how long (generations) to run the simulation; *as well as "bottleneck time" and "bottleneck pop. size". Don't worry about these last two, but be sure to set "bottleneck time" to be greater or equal to "generations".
...
## 3.1.1 How we infer the number of mutations, thinking of whole genomes



```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/Pisgenome.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/BayonaPicture2.png')
```
**Fig.3-3. Cost per base pair on different DNA sequencing platforms; figure by N. Bayona-Vasquez, 2018 - things changing SO fast though, can now (2020) get 110gigabases of data for <$1500 (e.g. HiSeqX at novogene.com `r emo::ji("smile")`) but Sanger sequencing costs have not improved `r emo::ji("frown")` **
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/coal1.jpg')
```
**Fig.3-x. Coalescent simulator; the SHiny app is below. **


```{r minicoalescent} 
shinyAppFile("/Users/johnwares/Dropbox/MolEcolText/shiny_popgen-master/Coalescence/discrete_time_app.R",options=list(width="100%",height=600))
...
shinyAppFile("/Users/johnwares/Dropbox/MolEcolText/shiny_popgen-master/Coalescence/continuous_coalescence_app.R",options=list(width="100%",height=600))
...
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/chydorus.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/recomb.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/hudsoncoyne02.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/Battey1.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/Wares16fig1.jpg')
```
**Fig.4-1. Molecular data are used to provide evidence that salmon from different rivers, and from different annual runs (odd years and even years), represent distinct populations of the same species. Figure from Wares (2016). **
...
Simpson's *D* is just one measure in a family of statistics known as Hill numbers (and lets not get into how many times *D* is used as a statistic in both fields!!), where the exponent of *ln*-summed frequencies increases the emphasis on common diversity over rare diversity in metrics as the exponent increases. We will return to these later as we discuss the 'effective' number of alleles, or fathers, or other elements of an ecological or evolutionary system. Of note, however, is that the definition above for the variance attributable to within-species variability, 1-*D*, is identical to "gene diversity" or heterozygosity as we evaluate genomic markers (Nei 1987).

Additionally, many of the approaches used to look at evenness relative to richness (like Fisher's $\alpha$) will return as we look at the frequency spectrum for polymorphic sites in genomic data relative to an underlying model of expected *lack* of evenness; these are also a big part of Hubbell's neutral theory of biodiversity, which also borrowed from population genetics the compound parameter $\theta$, a variable and interesting character who will make their appearance frequently in this book as we continue into genomic diversity. 

### 4.1.2 With molecular markers, what kind of a model can we include?

We have already noted two basic models for describing the diversity found in a sample: (W) and $\pi$ are estimators that rely on distinct sets of assumptions to represent the population mutation rate. In a single randomly mating population, when the diversity represents no fitness advantage or disadvantage, these estimators should be equivalent.

Another example can be considered now that we are more comfortable with diploid genomes and looking at distinct markers - whether those are variable nucleotide positions in a sequence, the aggregation of variation among sequences, or simplified markers that summarize genomic variation as variation in fragment length (microsatellites or restriction-digest based markers) or pattern of electrophoretic migration (allozymes). Now, we are dealing with an *equilibrium model*, where we can make predictions about future generations to the extent that our assumptions are not violated. That's the bonus we get with studying heritable data. In the purely ecological case, the diversity at a site is governed by the environment alone. In the genomic case, the diversity at a site is driven by both environmental variation and the genomic diversity that has been there and can persist there. And that lets us set up some simple predictions about what the most basic model of persistence of diversity would look like. 
...
Often, you'll see one additional evolutionary assumption, that the population is practically infinite in size. Since that is never true, and genetic drift alone will practically never (I think?) lead to deviations from predictions of this model, I'm going to jettison that right here. As noted before we will actually rely on drift in some cases as a means of estimating effective population sizes.
...
```{r, out.width='50%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/hosler1.jpg')
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/hosler23.jpg')
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/hosler4.jpg')
```
...
That may not help, but it will make more sense as we see more examples.

</div>


So, each of these distinct sequences (distinct in actual sequence, or size, or electrophoresis pattern, right?) that gets sampled is an *allele* with frequency $\ p_{i}$, with all frequencies summing to 1. At diploid, biparentally-inherited gene regions, we thus have a simple expectation for how these alleles are recombined through random sexual reproduction to generate *genotypes*. We expect individuals $\sum_{i} p_{i}^{2}$ to be homozygous, meaning both alleles have the same identity, and 1-($\sum_{i} p_{i}^{2}$) to be the proportion of individuals that carry two distinguishable alleles. We can directly observe allele frequencies by collecting genomic data, and these frequencies lead to a prediction of how they will be observed in genotypes. That's it. That's the model. It will hold as long as our 5 assumptions are true, and in general is robust to some assumptions being false as long as they don't lead to major bias in the outcome.

```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/khanpunnett.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/crayfishPD.jpg')
```
...
One simple example of why phylogenetic diversity may matter for making conservation or management decisions is shown in Fig 4-5. Knowing that locations carry distinct diversity is robust and visually determined in this example, but often it will be more complicated to tell the diversity apart from different sites, as with the microbial 16S diversity shown in Figure 3-x. However, the branch lengths in a phylogeny can be used to tell us about how much shared diversity there is between two or more samples. The metric 'phylogenetic diversity' or PD is actually measured by summing the branch lengths of a sample. If distinct samples have very distinct diversity - as in the Bray-Curtis plot above - then the PD of the combined "total" samples is much higher than the PD of each individual sample. This is statistically tested using permutation.
...
Almost all measures of differentiation of populations in a hierarchical framework are based on asking about the difference between overall diversity and mean diversity at a lower hierarchical scale, normalized by the overall diversity to produce a metric ranging (typically) from 0 to 1 (Wares 2016). Commonly (but often inaccurately) referred to as 'F-statistics' and derived from Wright's inbreeding coefficient *F* (1-(Ho/He), itself a contrast between observed and expected heterozygosity). The variations in how this is calculated, and how it is referred to, depend on the type of genomic data and the underlying model of how we assess the relationships among alleles, partly summarized here: https://www.molecularecologist.com/2011/03/should-i-use-fst-gst-or-d-2/ and partly in the Wares (2016) reference (which you 2020 students have read to learn about why distinct models apply to different types of data, in part because of the underlying way in which mutations are counted).

The basic idea of these F-statistics is that the diversity is described in hierarchical levels: individuals in a sampled location, samples in a region, regions in the overall (total) distribution, for example. And, in one way or another they are asking about the proportional amount of genetic variation in a lower level of the hierarchy relative to a higher level. For example, if allele frequencies are the same in two locations (try this), then the amount and type of diversity in both locations are the same, and the diversity can all be found in the site/sample level as much as in the overall (total) sample. We will get more involved with these statistics soon and the appropriate usage, for now you should try seeing how distinct allele frequencies (at a single, bi-allelic marker) in two locations lead to different $F_{ST}$ for the samples (here, F is appropriate for evaluating a single, bi-allelic marker; 'S' is for sample, and 'T' is for the total diversity; the metric $F_{ST}$ therefore tells us about the extent to which the diversity in each sample is only a subset of the total diversity, or *relatively more inbred*).

```{r firstlookfst} 


shinyAppFile("/Users/johnwares/Dropbox/MolEcolText/shiny_popgen-master/FST/fst_app.R",options=list(width="130%",height=500))
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/5drift5mig.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/wp08_1.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/wp08_2c.jpg')
```
...
What is important to recognize about the number of sites samples is that it provides resolution into the nature of hierarchy and dispersal that affect the distribution of genomic diversity and the potential for adaptation. Sampling completely continuously is not possible and is actually mathematically complicated (Wilkins & Wakeley 2002), but recognizing that more spatial samples enable the distinction between equilibrium and hierarchical models in a numbrer of ways means this is another element in determining the cost - in labor and funding - of a project like this.
...
I'm serious. Some programs (e.g. **Arlequin**) will refer to this with the subscript *R*, but in general you will see an $F_{CT}$ statistic reported; when this value is close to zero it means there is little "structure" or inbreeding associated with the samples within regions and so those regions do an adequate job of describing that there is hierarchical structure; as it deviates from zero it suggests more regions may need to be delimited.
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/micewahlund.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/Bradburdfig1.jpg')
```
...
# 6.1 Approaching reality, at least the "neutral" version of it...

```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/mosquitoes_Nc_Ne_ohmy.png')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/pangolinLG.jpg')
```
...
```{r, out.width='90%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('/Users/johnwares/Dropbox/MolEcolText/MEImages/EdwardsBeerli1.jpg')
```
...
Asymmetric movement and programs like Migrate or that paper that looks at parent-offspring dyads. Pop growth etc is in Chap 7 don't worry about it here. Get into dispersal kernels and isolation-by-resistance
...
movement is the difficult part. Not mapping or env layers or even currents (though some areas harder than others). It’s the REALITY of movement that we may hope to capture. That’s hard. I will present to a certain depth bit as always there are more complete treatments on landscape and seascape and benthicscape or whatever genetics. Genomics. Whatever. Anoles and storms, currents and asymmetry (Wares et al), asymmetry and selection (Carcinus), the trick is that movement is not always effective. Remember some models are useful, all are wrong. In the intent to find the *description* or model of movement - like vectors, with directions and magnitudes - our ability to generate complex models can reflect a lot about general patterns in nature through seeing how the environment forces diversity to move in certain ways. Wares et al (2001) recognized that the intraspecific diversity of a species will often reflect the environmental dynamics that lead to broader, community-level transitions. 


## 6.4 Summary

Last unit of chapter would maybe just summarize and organize all these notes, or perhaps focus us towards reading some example papers and making sense of them given our experience. Also don't forget the POPPR, now time to add "Part III" https://grunwaldlab.github.io/Population_Genetics_in_R/ and then after chapter 6 we are ready to get into the nitty gritty and read more :)

Ch6 I just realized landscape genomics replaces phylogeography bc we aren’t asking about monophyly but detail, Ibd and adaptation. LEA, ConStruct, Structure, DAPC - these are ways of generating understanding about movement and isolation but often not an answer on their own (sometimes in combination!)

Chapter 6 landscape genetics will get us (after reading that paper) to THIS https://onlinelibrary.wiley.com/doi/full/10.1111/1755-0998.13043 so that now we are really talking about the spatial importance of diversity.... there is a continuum of IBD and hierarchy and selection and demography and history that we are trying to build, here local Ne is very distinct from our understanding of "how many"

LEA software - ha now I use it. this is not NEW it is how the field has evolved, as I thought :)

HEY THIS CHAPTER COULD BOTH HANDLE LANDSCAPE/SEASCAPE GENETICS BUT ALSO THE REALISM OF WHEN THINGS SPLIT VIA IMPROVEMENTS ON MUTATION RATE, BRINGING IN THE PHYLOGEOGRAPHY IDEA FOR HOW IMPORTANT A GEOGRAPHIC EVENT IS AND PULLING IN HICKERSON - NOT JUST LINING UP PATTERNS BUT STATISTICAL PHYLOGEOGRAPHY, AND MAKE THAT DISTINCTION... LETS YOU EXPAND ON THESE ABC APPROACHES ETC AFTER YOU INTRODUCE THEM IN THIS CHAPTER. HOW WE TEST THE FIT OF MODELS CAN BE KEY.

Book: the utility of Philo geography is not the overuse of the word but the reverse fact with mutation rate that we can date things based on the diversions of cryptic lineage is otherwise they’re just cryptic lineages but it’s useful in the context of biogeography when you get overlap between the patterns and interesting when you don’t as in Wares 2002￼
Ch6 landscape seascape genetics and hypothesis testing

# 7. The relationship of alleles and inference of the history of populations

Here get into model rejection (tajD etc), ABC, separate populations, time, and so on...also Ne estimators, skyline plots etc. - this is where th eLuikart paper could come back in and the Roman & Palumbi
and definitely John Robinson and *Daphnia*


# 8 evaluating selection once you know how things are demographically linked, and how change in pop size is linked
Ch7 evaluating selection ONCE YOU KNOW HOW THINHS MOVE
the idea being that if most of the genome says X and a subset says Y they might be under different rules
...
# 9. Complicated versions of all fo this
...
# 10 RNA and methylation as components of adaptation

# 11 pop gen x quant gen
Shiny app for this!

# 12 EDGE dynamics and range shift variance in alleles, growth

# 13. kinship, parentage (Hill humbers, discrimination and assignment)

# 14 more behavior and interaction eg voles and mating, diversity and disease

# 15. data never fail to surprise.
...
Still working on all proper citations but already have much thanks to Silas Tittes for Shiny apps, to Justin Bagley and many others for help with R code checking, to my entire lab group in 2019-2020 for their patience, to my entire Molecular Ecology class in 2020 for *their* patience, to Natalia Bayona-Vasquez and Todd Pierson for their assistance getting me worked towards the experiential mode of teachign this class, 
# dangling notes
...
*notes to self: What about a chapter on somatic mutations and clonality - new paper you JUST TWEETED today (3/13 ish) on estimating mutaiton rate in a tree, and Don Levitan, and Sarah Otto, and other studies of mutation accumulaton, and fungus size - along with biogeography, should follow right after the community and “phylogeography” bit because the dating, the µ,  comes from that. Mutational differences are differences i time - and we are trying to use our best modes of inference to understand that relationship. That’s it. Panama, Arctic, Koskinen (as an example of where divergence and evolution of traits gets involved), red drum, whatever. Matt Hare and evolution per genertaion via drift. Put all of this as the stuff that gets in front of the equilibriuam stuff, the sampling-effective stuff. So this is the µ and t chapter, and … this hjust sets up variation in repro success. It does feel like all of this has to come before the silly pop gen stuff that you can make inference from.*

# Appendix 1. Session Information so this persists for a bit.
