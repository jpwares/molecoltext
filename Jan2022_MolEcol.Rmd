...
*add example figure here and in lectures*
(thanks for enduring the draftiness of this draft folks... this will probably be a comparison of *dominance* of a single species *Mytilus trossulus* when a community is manipulated by predator removal e.g. Paine and more recently Sarah Gravem)

```{r, out.width='50%', fig.align='center', fig.cap='...',echo=FALSE}
knitr::include_graphics('MEImages/Pisaster_Mytilus_dominance.jpg')

```
**Fig.4-1.5. Part of my BIOL 1104 teaching, figure from Bertness (1999) - I think (no, darnit). When we talk about dominance patterns in a community, a great example is the dominance of blue mussels (Mytilus) following predator removal in this Pacific Northwest rocky intertidal ecosystem. The number of species present (rather, being counted) has not changed but the evenness is much lower, which leads to a higher "inverse Simpson" value.**
...
*something about studying heterozygote diversity across spatial scales like North Atlantic work, simple?*
...
Now, we are dealing with an *equilibrium model*, where we can make predictions about future generations to the extent that our assumptions are not violated. That's the bonus we get with studying heritable data. In the purely ecological case, the diversity at a site is governed by the environment alone. In the evolutionary instance, the diversity at a site is driven by both environmental variation and the genomic diversity that *has been there* and *can persist there*. And that lets us set up some simple predictions about what the most basic model of persistence of diversity would look like. 
...
## 4.2 Diversity compared across samples. Finally. I know, it has been a lot of time talking about only one sample of diversity.
...
One simple example of why phylogenetic diversity may matter for making conservation or management decisions is shown in Fig 4-5. Knowing that locations carry distinct diversity is robust and visually determined in this example, but often it will be more complicated to tell the diversity apart from different sites, as with the microbial 16S diversity shown in **Figure 2.5**. However, the branch lengths in a phylogeny can be used to tell us about how much shared diversity there is between two or more samples. The metric 'phylogenetic diversity' or PD is actually measured by summing the branch lengths of a sample. If distinct samples have very distinct diversity - as in the Bray-Curtis plot above - then the PD of the combined "total" samples is much higher than the PD of each individual sample. This is statistically tested using permutation.
