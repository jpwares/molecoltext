...
**FOR ASU STUDENTS the below is not our goal right now but once you have access to Geneious, I'd first recommend taking the self-tour of what it can do and then writing me to ask any questions you have!**
...
library(learnPopGen)
driftplot<-drift.selection(p0=0.2,Ne=500,w=c(1,1,1),ngen=1000,nrep=5)
...
![**Fig.3-3. Cost per base pair on different DNA sequencing platforms; figure by N. Bayona-Vasquez, 2018 - things changing SO fast though, can now (2020) get 110gigabases of data for <$1500 (e.g. HiSeqX at novogene.com :) but Sanger sequencing costs have not improved :(  **](MEImages/BayonaPicture2.png)
...
Hopefully, we end up with successful reactions to create *DNA barcodes* for each juvenile. 

![**Fig.3-1. a barcode on a box of cereal tells the grocery store not just that it is cereal, but which cereal and even which size of box! **](MEImages/WMT-UPC-articleLarge.jpg)
Of course, your data will instead look like this: 

**A C T C T T G A T C G A G G G ** and so on, which actually looks like this:

![**Fig.3-2. Raw DNA sequence data **](MEImages/Good_chromatogram.jpg)

...and which we will work to look over for problems with those data (no method is totally perfect!), and then save the edited sequence and BLAST the data against the National Center for Biotechnology Information (NCBI). This massive library or database has many many billions of DNA sequences, whole genomes, RNA "transcriptomes" and more identified by species and location and experimental treatment.

We use this massive database, often called GenBank, as a way to interpret the DNA barcode we get from your mussels. This only works because (see back in Box A about reference libraries for environmental DNA) for *decades*, pretty much every DNA-based study ever done has been required to submit their data to this system. Sometimes that means sequences from an experimental mouse, but for the diverse ways that molecular ecologists have studied the *distribution, movement, and abundance of biodiversity* that also means there are millions of sequences for OUR TARGET GENE (mitochondrial COI) alone, across many tens of thousands of species. Those data were collected for other reasons, often, such as identifying the genetic distance between species noted above - but now they represent actual barcodes from those individuals, at that place, at that time.

When we BLAST a sequence we are using the *Basic Local Alignment Search Tool*. It is a computationally quick way to take any single sequence of DNA bases (...TTCTGAGTATTGGC...) and shuffle through the whole deck of all of the genomic sequence data it has (billlllllions of sequences) and quickly find the ones that offer the closest fit.

<style>
div.rose { background-color:#ffc4ef; border-radius: 10px; padding: 40px;}
</style>
<div class = "rose">
An example I like to use to get students to think about aligning "words" or strings of letters like this:
...
make an argument for which pair of words are more similar to each other than to the third word......

Remember that mutations (sec 2.5) can *change* a letter in DNA, or they can *add or remove* letters from a sequence. How would you develop 'rules' that let you determine which 2 words are most similar above, and how would you test those rules?

</div>

There is a link above (sec 2.3) that explains BLAST in detail, for now I just want you to think about how it works. By studying how mutations accumulate in either lab experiments (yeast, mice), or between diverging organisms on either side of an ancient barrier like the Isthmus of Panama, people studying DNA sequence data have figured out that certain types or patterns of mutations are more often than others, and so they can develop more detailed models that help a computer decide **very quickly** how 'similar' a DNA sequence is to another, and to give it a score (or a set of scores) to rank how similar it is.

That is what you did with those 3 words. That's what we will do with DNA sequences you generate that are 'words' or 'strings' with about 650-700 letters!! Fortunately, we have computers that can help us do this and we will be working to learn the program *Geneious* when we can meet in the next few weeks.
...
If this sounds like a lot to process, let me ask: have you ever made chocolate chip cookies following the recipe on the bag?

![**Fig.4-1. yes, I have screwed up this recipe when I was a kid so its OK if things don't go right the first time, but it isn't much harder than this **](MEImages/tollhouse.jpg)
...
# thank you for reading - *please* send comments along by email if there are things that are confusing. Some of what is in these first couple of chapters taken from my text would be in the context of a regular interaction/workshop, and some of it just plain needs to be written better! -jpw